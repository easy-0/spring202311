...
public class JoinValidator implements Validator<Member>, RequiredValidator {
    ...
    // 자바에선 클래스의 내부 구성 요소로 확장하는것이 일반적인 방식(패턴) / 상속x 기능확장에 불리하다.
    // private MemberDao dao = new MemberDao(); // 하지만 직접 객체 생성도 좋지 않다.
    @Autowired
    private MemberDao memberDao;

    public JoinValidator() {}

    // 의존관계 검증을 위해 MemberDao가 필요하다. = 전략패턴(변화에 닫혀있고-캡슐화, 확장에 열려있는 구조)
    ...
    public void validate(Member member){
        ...
        checkRequired(userName, new BadRequestException("회원명을 확인하세요."));
        ...
        // 비밀번호, 비밀번호 확인시 일치 여부
        // 문자열 비교는 equals로 비교! 주소가 아닌 값으로 비교
